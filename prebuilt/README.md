...
| File | What | Bound to |
|---|---|---|
| `panel_dash` | Dashboard daemon (x86_64, links `libdrm.so.2`, glibc ≥2.41). Built from the modular source in [`../src/panel/`](../src/panel/) (`build.sh`). | Any Unraid ≥7.3.x |
...
cp panel_dash        /boot/config/plugins/ugreen-idx6011-pro/panel/panel_dash
...
(vermagic mismatch; the panel stays dark, nothing else breaks). `panel_dash` itself
survives upgrades (rebuild only if you change the source under `../src/panel/`).
