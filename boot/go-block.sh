...
if [ -f $UGP/panel_dash ]; then
  ...
  cp $UGP/panel_dash /usr/local/bin/panel_dash
  chmod +x /usr/local/bin/panel_dash
  BG=$UGP/wallpaper.png
  [ -f "$BG" ] && BGA="--bg $BG" || BGA=""
  ( sleep 5; setsid /usr/local/bin/panel_dash --backlight 75 --interval 1 $BGA </dev/null >>/var/log/panel_dash.log 2>&1 ) &
fi
