...
static void card(int y, int h, const char *title){
    rect(10, y, W - 20, h, UN_GREY_80, g_card_opacity_eff * 255 / 100);   /* fill (wallpaper shows through when < 100) */
    rect(10, y, W - 20, 1, UN_GREY_70, 255);
    rect(10, y, 3, h, UN_ORANGE_M, 255);          /* webGUI-style orange spine */
    if (title) htext(22, y + gy(10), TITLE_SCALE, UN_TITLE, title);   /* section heading */
}
...
static void tile(int x, int y, int w, int h, const char *label){
    ...
    htext(x + 10, y + gy(8), TITLE_SCALE, UN_TITLE, label);   /* tile heading */
}
