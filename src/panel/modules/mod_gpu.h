 ...
 *
 * "gpu" module — iGPU busy %. Variants: 0=bar, 1=graph (sparkline).
...
static int mod_gpu(int y, stats_t *st, int variant){
    ...
    int h;
    if (variant == 1) h = spark_card(y, "GPU", v, h_gpu, h_cnt, h_pos, UN_ORANGE);
    else              h = metric_card(y, "GPU", st->gpu_busy, v, 0);
    if (st->gpu_freq >= 0){
